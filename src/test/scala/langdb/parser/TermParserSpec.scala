...
class TermParserSpec extends FunSuite:
  ...
  test("parse integer literal") {
    val result = TermParser.parse("42")
    assertEquals(result, Right(Term.IntLit(42)))
  }

  test("parse string literal") {
    val result = TermParser.parse("\"hello\"")
    assertEquals(result, Right(Term.StringLit("hello")))
  }

  test("parse boolean literals") {
    assertEquals(TermParser.parse("true"), Right(Term.BoolLit(true)))
    assertEquals(TermParser.parse("false"), Right(Term.BoolLit(false)))
  }

  test("parse variable") {
    val result = TermParser.parse("x")
    assertEquals(result, Right(Term.Var("x")))
  }

  test("parse simple lambda") {
    val result = TermParser.parse("fn x: Int => x")
    assertEquals(result, Right(Term.Lambda("x", Type.IntType, Term.Var("x"))))
  }

  test("parse lambda with body expression") {
    val result = TermParser.parse("fn x: Int => x + 1")
    assertEquals(
      result,
  ...
  test("parse nested lambdas") {
    val result = TermParser.parse("fn x: Int => fn y: Int => x + y")
    assertEquals(
      result,
      Right(
        Term.Lambda(
  ...
  test("parse let binding") {
    val result = TermParser.parse("let x = 5 in x + 1")
    assertEquals(
      result,
  ...
  test("parse let with lambda value") {
    val result = TermParser.parse("let double = fn x: Int => x + x in double 3")
    assertEquals(
      result,
      Right(
        Term.Let(
  ...
  test("parse multiplication has higher precedence than addition") {
    val result = TermParser.parse("1 + 2 * 3")
    assertEquals(
      result,
  ...
  test("parse print") {
    val result = TermParser.parse("print 42")
    assertEquals(result, Right(Term.Print(Term.IntLit(42))))
  }

  test("parse println") {
    val result = TermParser.parse("println 42")
  ...
  test("parse higher-order function types") {
    val result = TermParser.parse("fn f: Int -> Int -> Int => f 1 2")
    assertEquals(
      result,
      Right(
        Term.Lambda(
  ...
  test("reject keywords as identifiers") {
    val result = TermParser.parse("fn")
